...
abstract class BaseDAO<T extends Entity> {
  ...
  Future<int> save(T entity) async {
    ...
    var id = await dbClient.insert(tableName, entity.toMap(),
        conflictAlgorithm: ConflictAlgorithm
            .replace); //Método rawInsert usar a instrução SQL para fazer a manipulação.
    print('id: $id');
    return id;
  }
  ...
  Future<List<T>> query(String sql, [List<dynamic> arguments]) async {
    var dbClient = await db;
    final values = await dbClient.rawQuery(sql, arguments);
    return values.map<T>((json) => fromMap(json)).toList();
  }
